  ...
  nav_panel(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        selectizeInput("muni", "Municípios (destaque na tabela)", multiple = TRUE,
                       choices = sort(unique(base$nome_municipio)),
                       options = list(placeholder = "todos")),
        helpText("Base município × ano (7.095 linhas, 70 variáveis). Coberturas fixas",
      ...
      layout_columns(
        ...
        card(card_header("Série temporal (estado)"), plotlyOutput("trend", height = 420))
      ),
      layout_columns(
        col_widths = c(5, 7),
        card(card_header("Top 15 municípios no ano"), plotlyOutput("rank", height = 380)),
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        selectizeInput("m_muni", "Municípios", multiple = TRUE,
                       ...
                       options = list(placeholder = "todos")),
        uiOutput("dim_ui"),
        helpText("Filtragem preguiçosa via arrow — o SISAGUA (3,3 M linhas) é",
...
server <- function(input, output, session) {
  ...
  # base filtered to the selected year
  year_slice <- reactive(base |> filter(ano == input$ano))
  ...
  output$trend <- renderPlotly({
    ...
    ylab <- if (agg == "sum") "Total estadual" else "Mediana municipal"
