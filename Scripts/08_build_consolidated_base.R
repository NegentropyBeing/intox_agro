...
# Municipality names from SIDRA race file (all 645 SP municipalities, no network needed)
raca_sid <- read_excel("Bancos/pop_raca_cor_2022.xlsx", sheet = 1,
                       skip = 6, col_names = FALSE)
mun_names <- raca_sid |>
  filter(grepl("^35", ...1)) |>
  transmute(
    cod_ibge       = substr(...1, 1, 6),
    nome_municipio = sub(" \\(SP\\)$", "", ...2)
  )
rm(raca_sid)
...
# The IVS contextual file has multiple rows per municipality: the source crosses
# race (label_cor) by sex (label_sexo), each split by household situation
# (label_sit_dom). The overall municipal value is the "Total" cell of all three.
# Select it explicitly (this yields exactly one row per municipality).
...
if (anyDuplicated(ivs_raw$cod_ibge)) {
  stop("IVS: expected one 'Total Cor x Total Sexo' row per municipality, found duplicates")
}
...
# --- 4f. Racial composition (IBGE Census 2022) ---
pop_raca_cor <- read_parquet("resultados/contextual/pop_raca_cor_sp_2022.parquet")

cat("Racial composition:", nrow(pop_raca_cor), "municipalities\n\n")
...
pop_raca_cor  <- coerce_cod(pop_raca_cor)
...
base <- spine |>
  ...
  left_join(pop_raca_cor,  by = "cod_ibge")
...
vars_raca_cor <- c(
  "pct_branca",    # % White population (IBGE Census 2022)
  "pct_preta",     # % Black population (IBGE Census 2022)
  "pct_amarela",   # % Yellow/Asian population (IBGE Census 2022)
  "pct_parda",     # % Mixed-race population (IBGE Census 2022)
  "pct_indigena"   # % Indigenous population (IBGE Census 2022)
)
...
vars_final <- c(
  ...
  vars_raca_cor         # comment to exclude
)
